...
struct EffectsPage: View {
    ...
    var body: some View {
        LazyVGrid(columns: columns, spacing: 10) {
            ...
            card("Compressor", isOn: $effects.compressor.isOn) {
            ...
            card("Delay", isOn: $effects.delay.isOn) {
            ...
            card("Widener", isOn: $effects.widener.isOn) {
                SmallKnob("Width", $effects.widener.width, 0...2, percent: true, reset: 1)
            }
            card("Bass", isOn: $effects.bassEnhancer.isOn) {
    ...
    private func card<Knobs: View>(_ title: String, isOn: Binding<Bool>, @ViewBuilder knobs: () -> Knobs) -> some View {
        VStack(spacing: 8) {
            HStack {
